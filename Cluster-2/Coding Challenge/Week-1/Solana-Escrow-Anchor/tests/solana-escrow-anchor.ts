...
describe("solana-escrow-anchor", () => {
  ...
  const program = anchor.workspace
    .SolanaEscrowAnchor as Program<SolanaEscrowAnchor>;

  const ESCROW_PDA_SEED = "escrow";

  it("Setup", async () => {
    const _createMint = (payer: Signer) => {
      return createMint(provider.connection, payer, payer.publicKey, null, 0);
    };
    ...
    const exchangeTx = await program.rpc.exchange(
      new anchor.BN(terms.bobExpectedAmount),
      {
        accounts: {
          ...
          initializersTokenToReceiveAccount:
            escrow.initializerTokenToReceiveAccountPubkey,
